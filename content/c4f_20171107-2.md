...
然後執行, 表示要將 Github 上的倉儲 clone 到近端, 並且存在 cf4 目錄下.
...
接著利用 Leo Editor 開啟位於 y:\tmp\cf4\users 目錄下的 user.leo
...
因為近端格式沒有 Didqus 設定, 且未綁定遠端主機的網站符號名稱, 因此可以在近端以瀏覽器檢查內容, 若基本內容無誤後, 則可利用 remote pelican 按鈕轉為遠端格式後, 便可新增、提交與推送的指令.
